...
describe('MenuResolver', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    browseService = jasmine.createSpyObj('browseService', {
      getBrowseDefinitions: createSuccessfulRemoteDataObject$(
        createPaginatedList(BROWSE_DEFINITIONS)
      ),
    });
  ...
  describe('resolve', () => {
    ...
    it('should return an Observable that emits true as soon as all menus are created', () => {
      ...
      spyOn(resolver, 'createAdminMenu$').and.returnValue(
        cold('----(t|)', BOOLEAN)
      );
  ...
  describe('createPublicMenu$', () => {
    it('should retrieve the menu by ID return an Observable that emits true as soon as it is created', () => {
      (menuService as any).getMenu.and.returnValue(
        cold('--u--m--', {
          u: undefined,
          m: MENU_STATE,
        })
      ...
      expect(resolver.createPublicMenu$()).toBeObservable(
        cold('-----(t|)', BOOLEAN)
      );
    ...
    describe('contents', () => {
      ...
      it('should include browse dropdown', () => {
        expect(menuService.addSection).toHaveBeenCalledWith(
          MenuID.PUBLIC,
          jasmine.objectContaining({
            id: 'browse_global_by_definition1',
        ...
        expect(menuService.addSection).toHaveBeenCalledWith(
          MenuID.PUBLIC,
          jasmine.objectContaining({
            id: 'browse_global_by_definition2',
            parentID: 'browse_global',
            visible: true,
          })
        );
        expect(menuService.addSection).toHaveBeenCalledWith(
          MenuID.PUBLIC,
          jasmine.objectContaining({
            id: 'browse_global_by_definition3',
            parentID: 'browse_global',
            visible: true,
          })
        );
  ...
  describe('createAdminMenu$', () => {
    ...
    it('should retrieve the menu by ID return an Observable that emits true as soon as it is created', () => {
      ...
      expect(resolver.createAdminMenu$()).toBeObservable(
        cold('-----(t|)', BOOLEAN)
      );
