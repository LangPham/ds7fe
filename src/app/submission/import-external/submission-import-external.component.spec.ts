...
describe('SubmissionImportExternalComponent test suite', () => {
  ...
  describe('handle backend response for search query', () => {
              ...
              'dc.subject': [
                {
                  ...
                  language: null,
                  value:
                    'biological products | biotechnology | mycology | soursop',
                  place: -1,
                  authority: null,
                  confidence: -1,
                },
      ...
      errors: [
        {
          ...
          paths: ['/server/api/integration/externalsources/pubmed/entries'],
        },
      ],
    };
    beforeEach(() => {
    ...
    it('REST endpoint returns a 200 response with valid content', () => {
      mockExternalSourceService.getExternalSourceEntries.and.returnValue(
        createSuccessfulRemoteDataObject$(paginatedData.payload)
      );
      const expectedEntries = createSuccessfulRemoteDataObject(
        paginatedData.payload
      );
      spyOn(routeServiceStub, 'getQueryParameterValue').and.callFake(
        (param) => {
          if (param === 'entity') {
    ...
    it('REST endpoint returns a 200 response with no results', () => {
      ...
      expect(comp.isLoading$.value).toBe(false);
      expect(comp.entriesRD$.value).toEqual(expectedEntries);
      const noDataAlert = fixture.debugElement.query(
        By.css('[data-test="empty-external-entry-list"]')
      );
    ...
    it('REST endpoint returns a 500 error', () => {
      ...
      const noDataAlert = fixture.debugElement.query(
        By.css('[data-test="empty-external-error-500"]')
      );
      expect(noDataAlert).toBeTruthy();
    });
  });
