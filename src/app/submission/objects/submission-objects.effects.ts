    ...
    this.actions$.pipe(
      ...
      map(([action, currentState]: [SaveSubmissionFormSuccessAction, any]) => {
        return this.parseSaveResponse(
          ...
          action.payload.submissionId,
          currentState.forms,
          action.payload.showNotifications,
          action.payload.showErrors
        );
      }),
  ...
  protected parseSaveResponse(
    ...
    if (isNotEmpty(response)) {
      ...
      response.forEach((item: WorkspaceItem | WorkflowItem) => {
        ...
        const sherpaPoliciesSectionId = findKey(
          currentState.sections,
          (section) => section.sectionType === SectionsType.SherpaPolicies
        );
        if (
          isNotUndefined(sherpaPoliciesSectionId) &&
          isNotEmpty(currentState.sections[sherpaPoliciesSectionId]?.data) &&
          isEmpty(sections[sherpaPoliciesSectionId])
        ) {
          mappedActions.push(
            new UpdateSectionDataAction(
              submissionId,
              sherpaPoliciesSectionId,
              null,
              [],
              []
            )
